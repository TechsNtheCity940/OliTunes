...
class TablatureGenerator:
    ...
    def _initialize_position_model(self):
        ...
        try:
            ...
            if os.path.exists(model_path):
                ...
                self.position_model = tf.keras.models.load_model(model_path)
                logger.info("Successfully loaded pre-trained fretboard position model")
    ...
    def _optimize_single_note_position(self, note: Dict[str, Any],
        ...
        elif use_ml_model and self.position_model:
            try:
                logger.info(f"Using machine learning model for position inference: note {note['midi']}")
                print("  • Using machine learning for position inference...")
                
                # Convert note to feature vector
                # Simple feature: normalized MIDI value, octave, and position in measure
                feature = np.array([[
                    note['midi'] / 127.0,  # Normalize MIDI value
                    (note['midi'] // 12) / 10.0,  # Normalize octave
                    (note['time'] % 4.0) / 4.0  # Normalize position in measure
                ]])
                
                logger.debug(f"ML input features: MIDI={note['midi']}, octave={(note['midi'] // 12)}, position={note['time'] % 4.0}")
                ...
                logger.info(f"ML model predicted: string={pred_string}, fret={pred_fret}")
        ...
        # Otherwise, prefer lower positions and thicker strings 
        else:
    ...
    def _apply_chord_shape(self, position_options: List[Dict[str, Any]], 
        ...
        if shape_key:
            ...
            for note_data in position_options:
                ...
                # If we couldn't assign, find any valid position
    ...
    def _create_ascii_tab(self, measures: List[List[Dict[str, Any]]]) -> str:
        ...
        for measure_idx, measure in enumerate(measures):
            ...
            if measure and measure[0].get('chord'):
                measure_header += f" | Chord: {measure[0]['chord']}"
